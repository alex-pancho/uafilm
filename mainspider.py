...
def add_mov(films):
    ...
    type_src = films.get('type_src', "КіноФільм")
    poster = films.get('poster', "")
    desc = films.get('desc', "")
    director = films.get('director', "")

    # print([name_ua, type_src, name_orig, poster, desc, year, director])
    # input("...")
...
def insert_link(li):
    sql = f"""SELECT * FROM "cinema_link" WHERE "kinoid"={li[0]} and "link"='{li[1]}'  """
    try:
        cursor.execute(sql)
    except sqlite3.OperationalError as e:
        print(e)
